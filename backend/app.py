...
@jwt_required() # <--- THE LOCK: Only logged-in users can trigger alerts
def trigger_alert():
    current_user_id = get_jwt_identity() # Automatically gets WHO is logged in
    data = request.json
    
    new_alert = AlertCase(
        user_id=current_user_id, # Accountability: knows exactly who pushed the button
    ...
    db.session.commit()
    return jsonify({"msg": "Alert raised securely"}), 201
